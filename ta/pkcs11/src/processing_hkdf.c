 ...
 *
 * HKDF (RFC 5869) on top of HMAC primitives. Built directly on TEE_MAC_*
 * rather than the consolidated TEE_ALG_HKDF so we can offer extract-only
 * and expand-only modes, and any HMAC-supported PRF hash, uniformly.
 */
...
struct hkdf_prf {
	uint32_t hmac_alg;	/* TEE_ALG_HMAC_* */
	uint32_t hmac_key_type;	/* TEE_TYPE_HMAC_* */
	uint32_t hmac_min_bits;	/* matches the HMAC_* type's min key size */
	uint32_t hmac_max_bits;	/* matches the HMAC_* type's max key size */
	uint32_t hash_len;	/* HashLen, in bytes */
};

/*
 * Min/max ranges mirror the GP HMAC type properties in
 * core/tee/tee_svc_cryp.c. The strict ranges (default vexpress-qemu, no
 * CFG_HMAC_64_1024_RANGE) are used here. Keys shorter than the minimum
 * are zero-padded by hmac_one_shot() to the minimum, which is
 * indistinguishable from the unpadded key per RFC 2104 (HMAC zero-pads
 * short keys internally).
 */
...
/*
 * Run HMAC-Hash(key, data1 || data2 || data3) and write the tag (always
 * HashLen bytes) to @out.
 *
 * The GP HMAC types impose a minimum key size that may be larger than
 * RFC 5869's smallest test vectors (e.g. SHA-256 minimum is 192 bits in
 * the strict range, but RFC 5869 A.1's salt is only 104 bits). RFC 2104
 * defines HMAC over short keys by zero-padding them to the block size,
 * so zero-padding here up to the GP minimum produces the same MAC as
 * the unpadded key would. Keys exceeding the type's max key size are
 * rejected; HKDF salts and PRKs are bounded by HashLen so we don't
 * realistically exercise that limit.
 */
 ...
 * If salt is empty, RFC requires a string of HashLen zero bytes. The
 * caller must provide a buffer that's at least @prf->hash_len when salt
 * is empty.
 */
 ...
 * with N = ceil(L / HashLen). N must be <= 255.
 */
...
/*
 * Wire format produced by libckteec serialize_mecha_hkdf_derive_param():
 *   u32 bExtract
 *   u32 bExpand
 *   u32 prfHashMechanism
 *   u32 ulSaltType   (PKCS11_CKF_HKDF_SALT_NULL/DATA/KEY)
 *   u32 ulSaltLen
				  ...
				  struct obj_attrs **head)
{
	...
	/*
	 * Reject zero-length output up front: TEE_Malloc(0, 0) is
	 * implementation-defined per GP, and a zero L is meaningless for
	 * both extract (must equal HashLen) and expand (RFC 5869 §2.3
	 * requires L >= 1).
	 */
	if (!out_byte_size) {
	...
	/*
	 * Mode-specific size constraints (RFC 5869):
	 *   - extract-only: output is always exactly HashLen.
	 *   - expand:       L <= 255 * HashLen (§2.3).
	 *   - expand-only:  IKM is treated as the PRK; must be >= HashLen.
	 */
	if (extract == PKCS11_TRUE && expand != PKCS11_TRUE &&
	...
	if (expand == PKCS11_TRUE) {
		...
		/* Extract-only: emit PRK directly. */
